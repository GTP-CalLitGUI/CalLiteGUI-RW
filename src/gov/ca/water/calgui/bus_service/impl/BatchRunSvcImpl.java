 ...
 */
public class BatchRunSvcImpl implements IBatchRunSvc {

	private static Logger log = Logger.getLogger(BatchRunSvcImpl.class.getName());
	...
	public void doBatch(List<String> scenarioNamesList, SwingEngine swingEngine, boolean isWsidi) {
		...
		if (isWsidi) {
			if (scenarioNamesList.size() > 1) {
				...
				setupMainBatchFile_WSIDI(null, scenarioNamesList.get(0), wsdiIterations);
				runBatch();
			}
		} else {
